...
if __name__ == '__main__':
    ...
    ### training options
    parser.add_argument('--iters', type=int, default=2500, help="training iters") # was 30000
    ...
    # parser.add_argument("--max_freeze_palette_epoch", type=int, default=10, help='epoch number when palette color is released')
    parser.add_argument("--smooth_loss_start_epoch", type=int, default=30, help='epoch number when smooth loss is added')
    
    parser.add_argument("--max_freeze_palette_epoch", type=int, default=0, help='15 epoch number when palette color is released')
    parser.add_argument("--max_fix_omega_epoch", type=int, default=15, help=' 15 fix omega after this epoch')
    
    parser.add_argument("--lambda_offsets", type=float, default=0.1, help='(was 0.1) weight of color offsets regularity loss')
    ...
    if opt.use_initialization_from_rgbxy:
        assert os.path.exists(os.path.join(palette_workspace, 'extract-palette.txt')), "Extracted palette is missing."
        # extracted_palette = np.load(os.path.join(palette_workspace, 'palette.npz'))['palette']
        extracted_palette = np.loadtxt(os.path.join(palette_workspace, 'extract-palette.txt'))
    ...
    # train PaletteNeRF 
        ...
        trainer = PaletteTrainer('palette', opt, model, device=device, workspace=workspace, optimizer=optimizer, criterion=criterion, 
            ...
            use_checkpoint=opt.ckpt, nerf_path=opt.nerf_path, eval_interval=28) # was 50
        test_loader = NeRFDataset(opt, device=device, type='test', n_test=30).dataloader()
        # train
