...
def similar_at(centeres, rgb):
    res = 0
    rgb_hsv = RGB2HSV(rgb)
    for i in range(centeres.shape[0]):
        dists = np.cos(np.dot(centeres[i], rgb) / (np.linalg.norm(centeres[i]) * np.linalg.norm(rgb)))
        # if np.abs(RGB2HSV(centeres[i])[0] - rgb_hsv[0]) < 0.1:
        if dists > 0.8:
            return res
        res += 1
    return res
...
def palette_extraction(
    ...
    bin_weights_coarse, bin_centers_coarse = compute_RGB_histogram(colors, weights, bits_per_channel=3) #was 3
    ...
    bin_weights_fine, bin_centers_fine = compute_RGB_histogram(colors, weights, bits_per_channel=5) #was 5
        ...
        if not any(np.array_equal(palette_rgb[color_idx], x) for x in new_palette_rgb): # if not in new palette
            if new_palette_rgb == []: # if new palette is empty
                ...
                for new_color_idx in range(len(new_palette_rgb)): # traverse new palette, compare with each color
                    ...
                    # dist = np.abs(RGB2HSV(palette_rgb[color_idx])[0] - RGB2HSV(new_palette_rgb[new_color_idx])[0])
                    dist = min(np.abs(RGB2HSV(palette_rgb[color_idx])[0] - RGB2HSV(new_palette_rgb[new_color_idx])[0]), 1 - np.abs(RGB2HSV(palette_rgb[color_idx])[0] - RGB2HSV(new_palette_rgb[new_color_idx])[0]))
                    # if dist > 0.95: # there is a similiar color already in new_palette, do not add
                    if dist < 0.1:
                        has_similiar = True
                        similiar_in = similar_at(centers, new_palette_rgb[new_color_idx])
                        similiar_out = similar_at(centers, palette_rgb[color_idx])
                        # ic(similiar_in, similiar_out)
                        if similiar_in > similiar_out:
                            # ic("exchange!")
                            new_palette_rgb[new_color_idx] = palette_rgb[color_idx]
                        break
                if not has_similiar:
                    #* remove low saturation color (very likely come from lighting)
                    if (RGB2HSV(palette_rgb[color_idx])[2] < 0.2):
                        break
    ...
    # ic(palette_rgb.shape)
    # ic(hist_weights.reshape(-1,palette_rgb.shape[0]).mean(0))
...
class PaletteTrainer(object):
    def __init__(self, 
        ...
        self.max_fix_omega_epoch = opt.max_fix_omega_epoch
    ...
    def train(self, train_loader, valid_loader, max_epochs):
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
            ...
            if epoch >= self.opt.max_fix_omega_epoch:
                self.model.freeze_omega()
                ...
                if self.local_rank == 0:
                    ...
                    if save_images:
                        ...
                        for i in range(self.opt.num_basis):
                            # was: basis_img = (outputs['basis_rgb'][0,:,i*3:(i+1)*3]).reshape(pred.shape[0], pred.shape[1], 3) 
                            
                            basis_img = outputs['basis_rgb'][i].reshape(pred.shape[0], pred.shape[1], 3) 
                            # ic(basis_img.shape)
                            # exit()
                            # basis_img = (outputs['basis_rgb'][0,:,i*3:(i+1)*3]+ (1 - outputs['weights_sum']).unsqueeze(-1)).reshape(pred.shape[0], pred.shape[1], 3) 
    ...
    def test(self, loader, save_path=None, name=None, write_video=True, selected_idx=None, gui_mode=False):
        ...
        if write_video:
            ...
            def mwrite(filename, frames):
                ...
                imageio.mimwrite(filename, frames, fps=30, quality=8, macro_block_size=1)
        ...
        if not best:
            ...
            if len(self.stats["results"]) > 0:
                if self.stats["best_result"] is None or self.stats["results"][-1] < self.stats["best_result"]:
                    ...
                    # ic('density_grid' in state['model'])
                    # exit()
