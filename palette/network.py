...
class PaletteNetwork(PaletteRenderer):
    ...
    def forward(self, x, d, soft2hard=False):
        ...
        omega, offsets_radiance, view_dep = self.color(x, d, geo_feat=geo_feat)
        ...
        return sigma, clip_feat, omega, offsets_radiance, view_dep
    ...
    def color(self, x, d, mask=None, geo_feat=None, **kwargs):
        ...
        # h_diffuse = torch.zeros_like(h_diffuse) #*todo: kt: remove diffuse color
        ...
        #     if l != self.num_layers_color - 1:
        #         h = F.relu(h, inplace=True)
        # h_view_dep = torch.sigmoid(h) #* was originally used
        ...
        # # h_view_dep = h.mean(dim=-1).unsqueeze(-1)*torch.ones((h.shape[0],3), dtype=h.dtype, device=h.device)
        # h_view_dep = h.sum(dim=-1).unsqueeze(-1)*torch.ones((h.shape[0],3), dtype=h.dtype, device=h.device)
