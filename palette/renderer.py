...
class PaletteRenderer(nn.Module):
    ...
    def initialize_palette(self, color_list=None, hist_weights=None):
        if color_list is None:
            ...
            if len(color_list.shape)==1: color_list = color_list.reshape(-1,1)
    ...
    def run_cuda(self, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, 
        ...
        if self.training:
            ...
            sigmas, clip_feat, omega, offsets_radiance, view_dep = self(xyzs, dirs)
            ...
            rgbs = basis_rgb.sum(dim=-2) + view_dep#.detach()
            ...
            all_buffer = torch.cat([omega_sparsity, view_dep_norm, offsets_norm, smooth_norm, 
                                    view_dep,clip_feat, omega[...,0]], axis=-1)
            ...
            clip_feat_map = all_map[...,7:7+self.opt.clip_dim]
            basis_acc_map = all_map[...,7+self.opt.clip_dim:7+self.opt.clip_dim+self.opt.num_basis]
            ...
            while step < max_steps:
                ...
                # sigmas, clip_feat, omega, offsets_radiance, view_dep, diffuse = self(xyzs, dirs, soft2hard=True)
                sigmas, clip_feat, omega, offsets_radiance, view_dep = self(xyzs, dirs, soft2hard=True)
                ...
                # ic(rgbs.shape) #(N_rays*N_samples_, 3)
                ...
                #分开渲染，分别设定ray_alive for each basis,使用composite_rays来得到 each weight map

                step += n_step
            basis_weights_sum,basis_depths_sum,basis_images = self.segment_res(rays_o, rays_d, nears, fars, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, 
                            max_steps=1024,T_thresh=1e-4)

            image = image + (1 - weights_sum).unsqueeze(-1) * bg_color #* this line make the result from blank bg to white bg
            ...
            if not gui_mode:
                ...
                results['basis_rgb'] = basis_images
                results['unscaled_basis_rgb'] = unscaled_basis_rgb_map
                results['basis_acc'] = basis_acc_map

        return results

    def segment_res(self,rays_o, rays_d, nears, fars, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, 
                 max_steps=1024,T_thresh=1e-4):
        dtype = torch.float32
        ...
        for i in range(self.opt.num_basis):
            ...
            while step < max_steps:
                ...
                xyzs, dirs, deltas = raymarching.march_rays(n_alive, n_step, rays_alive, rays_t, rays_o, rays_d, self.bound, self.density_bitfield, self.cascade, self.grid_size, nears, fars, 128, perturb if step == 0 else False, dt_gamma, max_steps)
                M = xyzs.shape[0]
                # sigmas, _, omega, offsets_radiance, view_dep, diffuse = self(xyzs, dirs)
                sigmas, _, omega, offsets_radiance, view_dep = self(xyzs, dirs)
                offsets, radiance = offsets_radiance[...,:-1], offsets_radiance[...,-1:]
                ...
                final_color = (F.softplus(radiance)*(basis_color+self.offsets_weight * offsets))# .clamp(0, 1)
                basis_rgb = (omega[:,i,:]*final_color[:,i,:]) +  self.view_dep_weight * view_dep #todo: with lighting
                # basis_rgb = (omega[:,i,:]*final_color[:,i,:]) #todo: without lighting
            ...
            basis_images[i] = (basis_images[i]+ (1 - basis_weights_sum[i]).unsqueeze(-1)) #* add white bg_color
                
        return basis_weights_sum,basis_depths_sum,basis_images
